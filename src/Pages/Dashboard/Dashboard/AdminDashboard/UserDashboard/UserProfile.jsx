      ...
      <div className='text-center my-5 text-slate-700 '><h1 className='text-3xl font-bold'>Profile</h1></div>
                ...
                <div className="flex items-center justify-center max-w-[300px] mx-auto relative">
                    <label className="flex flex-col items-center justify-center w-44 h-44 border-2 border-gray-300 border-dashed rounded-lg  bg-gray-50 hover:bg-gray-100 ">
                        ...
                        {
                            profile?.img ?
                                <img src={profile?.img} alt="" className='w-full h-full rounded' />
                                :
                                <>
                                    <span className="font-medium text-gray-100 ">
                                        {
                                            profile?.name &&
                                            profile?.name.split(" ").map((i) => i.charAt(0).toUpperCase()).join("")
                                        }
                ...
                <div className="mt-6 grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <p className="bg-gray-50 border-b border-gray-300 text-gray-900 text-sm  block w-full p-2.5  " >{profile?.name}</p>
                    </div>
                    <div>
                        <label htmlFor="last_name" className="block mb-2 text-sm font-medium text-gray-900 ">Coin</label>
                        <p className="bg-gray-50 border-b border-gray-300 text-gray-900 text-sm  block w-full p-2.5  " >{profile?.coin || 0}</p>
                    </div>


                    <div>
                        <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 ">Email address</label>
                        <p className="bg-gray-50 border-b border-gray-300 text-gray-900 text-sm  block w-full p-2.5  " >{profile?.email}</p>
                    </div>

                    {/* phone  */}
                    <div>
                        <label htmlFor="phone" className="block mb-2 text-sm font-medium text-gray-900 ">Phone number</label>
                        <p className="bg-gray-50 border-b border-gray-300 text-gray-900 text-sm  block w-full p-2.5  " >{profile?.phone || "N/A"}</p>
                    </div>
                </div>
                <div className="mb-6">

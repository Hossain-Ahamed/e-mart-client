...
const CheckOut = () => {
  ...
  const [discount, setDiscount] = useState({ couponCode: '', discountedAmmount: 0.0 });
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_SERVERADDRESS}/checkout?email=${user?.email}`, {
  ...
  const handleCoupon = data => {
    ...
    axios.post(`${import.meta.env.VITE_SERVERADDRESS}/get-discount-by-coupon?email=${user?.email}`, data, {
      ...
      .then((data) => {
        console.log(data.data);
        setDiscount({ couponCode: data.data?.couponCode, discountedAmmount: data.data?.discountedAmmount })
      })
      .catch((e) => {
  ...
  return (
    ...
    <AdminTitle heading="CheckOut"/>
      <div className="grid grid-cols-3 gap-5 m-10">
            ...
            <table className="min-w-full">
              ...
              <tbody>
                ...
                {selectedOrderItems.map((item, index) => (
                  <tr className=" user-select-none text-center border-b hover:bg-slate-100/75 " key={index}>
                    <td
                      scope="col"
        ...
        <div className="border rounded-md shadow-lg bg-white p-5 h">
          <div>

            <form onSubmit={handleSubmit(handleCoupon)}>
              <label htmlFor="default-search" className="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Search</label>
              <div className="relative">
                <input  type="text" {...register("couponName")}  autoComplete="off" id="default-search" className="block w-full p-3  text-base text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="Coupon Code" required />
                <button type="submit" disabled={serverRequesting} className="text-white absolute right-2.5 bottom-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 disabled:bg-slate-400">Apply</button>
              </div>
            </form>
            ...
            <div className="flex justify-between items-center px-1">
              <p>Discount:</p><p>- {discount?.discountedAmmount}</p>
            </div>
            <div className="flex justify-between items-center px-1">
              <p>Total Payment: </p> <p>{totalProductPrice + deliveryCharge - discount?.discountedAmmount}</p>
            </div>
          </div>
          <button className="w-full h-10 focus:ring focus:ring-3 ring-yellow-300  bg-accent text-white text-lg font-bold rounded-sm mt-10 disabled:bg-gray-500" disabled={serverRequesting}>Place Order</button>
        </div>
      </div>
    </>
  );
